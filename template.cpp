...
private:
    ...
    BinaryImg<T> Or(T rhs) const //не измен€емый экземпл€р класса
    {
        BinaryImg<T> result(row, col);
        for (int i = 0; i < row; i++)
        {
    ...
    BinaryImg<T> And(const BinaryImg<T>& src) const  //перегрузка методов
    {
...
public:
    ...
    BinaryImg(const BinaryImg<T>& src)  //конструктор копир
    {
        row = src.row;
        col = src.col;
        array = new T * [row];
    ...
    BinaryImg<T> operator+(const BinaryImg<T>& src) const  //оператор сложени€ двух матриц
    {
        return this->Or(src);
    }
    ...
    friend BinaryImg<T> operator+(T rhs, const BinaryImg<T>& src)
    {
        return src.Or(rhs);  //прибавл€ем число и возвращаем
    }
};
